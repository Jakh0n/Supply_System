...
export class PDFGenerator {
  ...
  static async generateOrdersPDF(
    ...
    // Orders by branch
    const branchSummary = orders.reduce(
      (acc, order) => {
        if (!acc[order.branch]) {
          acc[order.branch] = [];
        }
  ...
  /**
   * Branch orders monthly report — per branch, aggregated products for the month.
   */
  static async generateBranchOrdersMonthPDF(
    data: Record<
      string,
    ...
    const { orientation = "landscape", format = "a4", margin = 18 } = options;
    ...
    const periodText = `Report period: ${periodStartStr} – ${periodEndStr}`;
    ...
    const title = `Branch orders (monthly summary) — ${monthLabel} ${year}`;
    pdf.setFontSize(14);
    pdf.setFont("helvetica", "bold");
    pdf.text(title, margin, yPosition);
    yPosition += 10;
    ...
    if (branchNames.length === 0) {
      pdf.text("No orders in this period.", margin, yPosition);
      pdf.save(`branch-orders-${year}-${month}-${Date.now()}.pdf`);
      return;
    }
    ...
    for (const branchName of branchNames) {
      ...
      pdf.setFont("helvetica", "bold");
      pdf.setFontSize(11);
      pdf.text(`Branch: ${branchName}`, margin, yPosition);
      yPosition += 8;
      ...
      for (let i = 0; i < mid; i++) {
        ...
        const right = products[mid + i];
        const leftText = left
          ? `  • ${(left.name ?? "").substring(0, 28)} — ${left.quantity} ${left.unit}`
          : "";
        const rightText = right
          ? `  • ${(right.name ?? "").substring(0, 28)} — ${right.quantity} ${right.unit}`
          : "";
        if (leftText) pdf.text(leftText, xLeft, yPosition);
        if (rightText) pdf.text(rightText, xRight, yPosition);
    ...
    if (overallProducts.length > 0) {
      ...
      pdf.setFont("helvetica", "bold");
      pdf.setFontSize(12);
      pdf.text("Overall total (all branches)", margin, yPosition);
      yPosition += 10;
      pdf.setFont("helvetica", "normal");
      pdf.setFontSize(9);
      const midOverall = Math.ceil(overallProducts.length / 2);
      for (let i = 0; i < midOverall; i++) {
        checkNewPage(lineHeight);
        const left = overallProducts[i];
        const right = overallProducts[midOverall + i];
        const leftText = left
          ? `  • ${(left.name ?? "").substring(0, 28)} — ${left.quantity} ${left.unit}`
          : "";
        const rightText = right
    ...
    pdf.text(periodText, margin, footerY);
    pdf.text(
      `Generated: ${new Date().toLocaleDateString("en-GB")}`,
      pageWidth - margin - 50,
      footerY,
    ...
    const fileName = `branch-orders-${year}-${String(month).padStart(2, "0")}-${Date.now()}.pdf`;
    pdf.save(fileName);
  }
  ...
  static async generateWeeklyStatisticsPDF(
    ...
    // Orders by Status
    checkNewPage(30);
    const statusSummary = weeklyData.orders.reduce(
      (acc, order) => {
        acc[order.status] = (acc[order.status] || 0) + 1;
        return acc;
      },
  ...
  static async generateMonthlyStatisticsPDF(
    ...
    // Orders by Status
    checkNewPage(40);
    const statusSummary = monthlyData.orders.reduce(
      (acc, order) => {
        acc[order.status] = (acc[order.status] || 0) + 1;
        return acc;
      },
