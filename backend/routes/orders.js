...
// Korean calendar / holiday notes for workers — must be before /:id
router.get("/day-context", authenticate, requireWorker, async (req, res) => {
  try {
    const { requestedDate } = req.query;
    res.json(getOrderDayContext(requestedDate));
  } catch (error) {
    console.error("Order day context error:", error);
    res.status(500).json({
      message: "Server error fetching day context",
    });
  }
});
...
function buildEditorOrderFilter({ date, branch }) {
  const filter = {};
  if (date) {
    const startDate = new Date(date);
    const endDate = new Date(date);
...
// Bulk update all orders matching optional filters (admin/editor only)
router.patch(
  "/bulk/status-all",
  authenticate,
  requireAdminOrEditor,
  ...
  async (req, res) => {
    try {
      ...
      if (req.body.requestedDate) {
        ...
        const [year, month, day] = req.body.requestedDate
          .split("-")
          .map(Number);
